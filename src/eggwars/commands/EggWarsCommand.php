...
class EggWarsCommand extends Command implements PluginIdentifiableCommand {
    ...
    public function __construct() {
        parent::__construct("crystalwars", "CrystalWars commands", null, ["cw"]);
    ...
    public function checkPermission(CommandSender $sender, string $subcommandName) {
        return $sender->hasPermission("cw.cmd.$subcommandName");
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) {
        if(empty($args[0])) {
            if($sender->hasPermission("cw.cmd.help")) {
                $sender->sendMessage("§cUsage: §7/cw help");
            }
            else {
                $sender->sendMessage("§cUsage: §7/cw join §8| §7/cw leave");
            }
            return;
        }
        $name = $args[0];
