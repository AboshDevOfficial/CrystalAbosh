...
class TeamManager {
    ...
    public function onEggBreak(Player $player, Vector3 $eggVector): bool {
        ...
        if($team->inTeam($player)) {
            $player->sendMessage(EggWars::getPrefix()."§cYou can not broke you own crystal!");
            return true;
        }
        $team->setAlive(false);
        $this->getArena()->broadcastMessage("§7------------------== §8[§bProgress§8] §7==------------------\n".
            "§7Player {$player->getName()} from team ".$this->getArena()->getTeamByPlayer($player)->getDisplayName()." §7destroyed crystal of ".$team->getDisplayName()." §7team.\n".
            "§7---------------------------------------------------");
        return false;
    }
