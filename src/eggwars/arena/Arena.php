...
class Arena {
    ...
    public function joinPlayer(Player $player, $team = null) {
        ...
        $this->broadcastMessage(EggWars::getPrefix()."§a{$player->getName()} has joined §7[$count/$maxCount]!");
    ...
    public function lobby() {
        ...
        if($this->getFillTeamsCount() >= intval($this->arenaData["teamsToStart"])) {
            ...
            if(empty($this->progress["startTime"]) || !is_numeric($this->progress["startTime"])) {
                ...
                foreach ($this->getAllPlayers() as $player) {
                    $player->sendMessage(EggWars::getPrefix()."§7Starting in $startTime sec!");
                }
            }
            ...
            foreach ($this->getAllPlayers() as $player) {
                ...
                $player->sendPopup(EggWars::getPrefix()." §7 §aStarting in {$this->progress["startTime"]}");
            ...
            if(in_array($startTime, [120, 90, 60, 45, 30, 20 ,15, 10])) {
                $this->broadcastMessage(EggWars::getPrefix()."§7Starting in {$startTime} sec!");
            }
        }
        else {
            foreach ($this->getAllPlayers() as $player) {
                $player->sendPopup(EggWars::getPrefix()." §7 §cNeed more players!");
            }
        }
    }
