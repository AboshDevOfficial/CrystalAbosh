...
class GeneratorScheduler extends EggWarsTask implements Listener {
    ...
    // line 2
    const LINE_2_DIAMOND = "§aEmerald";
    const LINE_2_GOLD = "§bDiamond";
    const LINE_2_IRON = "§6Gold";
    ...
    public function getSigns(Level $level, int $material): array {
        ...
        foreach ($levelSigns as $sign) {
            if(($material == self::GOLD) && ($sign->getText()[1] == self::LINE_2_IRON)) {
                array_push($signs, $sign);
            }
            if(($material == self::DIAMOND) && ($sign->getText()[1] == self::LINE_2_GOLD)) {
                array_push($signs, $sign);
            }
            if(($material == self::EMERALD) && ($sign->getText()[1] == self::LINE_2_DIAMOND)) {
                array_push($signs, $sign);
            }
        }
        return $signs;
        ...
        foreach ($signs as $sign) {
            ...
            switch (strval($level)) {
                case "1":
                    // 5 sec
                    if($this->tick%100 == 0) {
                        $this->getArena()->getLevel()->dropItem($sign->asVector3()->add(0.5, 0, 0.5), Item::get(Item::DIAMOND), new Vector3());
                    }
                    break;
                case "2":
                    // 3 sec
    ...
    public function dropEmerald() {
        $signs = $this->getSigns($this->getArena()->getLevel(), self::EMERALD);
        foreach ($signs as $sign) {
            $level = intval(str_replace("§0Level ", "", $sign->getText()[2]));
            switch (strval($level)) {
                case "1":
                    // 20 sec
                    if($this->tick%400 == 0) {
                        $this->getArena()->getLevel()->dropItem($sign->asVector3(), Item::get(Item::EMERALD), new Vector3());
                    }
                    break;
                case "2":
                    // 15 sec
    ...
    private static function loadSign(Sign $sign) {
        ...
        $gen = $text[1];
        $lvl = $text[2];
        if(!in_array($gen, ["Diamond", "Gold", "Emerald"])) return;
    ...
    public function createUpdateWindow(Player $player, int $ingot, int $genLevel, Tile $tile) {
        ...
        $nbt = new CompoundTag('', [
            ...
            new StringTag('CustomName', "§6§lCrystalWars Generator"),
        ...
        switch ($ingot) {
            ...
            case self::DIAMOND:
                ...
                break;
            case self::EMERALD:
                $inv->setItem(11, Item::get(Item::EMERALD)->setCustomName("§aEmerald generator\n§blevel: {$genLevel}\n§btime: {$time} sec."));
                if(isset($this->ticks[$ingot][intval($genLevel+1)])) {
                    $inv->setItem(14, Item::get(Item::EXPERIENCE_BOTTLE)->setCustomName("§7UPDATE GENERATOR\n§bnext level: ".strval($genLevel+1)."\n§btime: ".$this->ticks[$ingot][intval($genLevel+1)]."sec.\n{$this->getPrice($ingot, $genLevel+1)}"));
                }
                else {
    ...
    public function getPrice($ingot, $genLevel): string {
        ...
        switch ($item->getId()) {
            ...
            case Item::EMERALD:
                return "§aEmerald §9{$item->getCount()}x";
            default:
                return "MAX LEVEL";
        }
    }
    ...
    public function getPriceItem($ingot, $genLevel): Item {
        switch ($ingot) {
            case self::GOLD:
                switch ($genLevel) {
                    case 1:
                        return Item::get(Item::GOLD_INGOT, 0, 20);
                    case 2:
            ...
            case self::DIAMOND:
                switch ($genLevel) {
                    ...
                    case 2:
                        return Item::get(Item::DIAMOND, 0, 40);
                    case 3:
                        return Item::get(Item::EMERALD, 0, 20);
                }
                break;
            case self::EMERALD:
                switch ($genLevel) {
                    case 0:
                        return Item::get(Item::EMERALD, 0, 20);
                    case 1:
                        return Item::get(Item::EMERALD, 0, 25);
                    case 2:
                        return Item::get(Item::EMERALD, 0, 30);
                }
                break;
